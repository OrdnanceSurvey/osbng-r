...
followed by `tidyr::unnest`. If the `sf` package is available, then
`geom_to_bng_intersection_explode()` provides this functionality directly.

The example code below shows an alternative approach without the dependency on `tidyr`.
